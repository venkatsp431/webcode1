...
async function fetchAPI() {
  ...
  try {
    ...
    if (brands.includes(searchResult)) {
      ...
      url = `http://makeup-api.herokuapp.com/api/v1/products.json?product_type=${input.value}`;
    } else if (names.includes(searchResult)) {
      url = `http://makeup-api.herokuapp.com/api/v1/products.json?name=${input.value}`;
    }
